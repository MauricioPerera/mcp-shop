...
export function registerSearchProducts(server: McpServer, env: Env) {
  server.registerTool(
    "search_products",
    {
      description:
    ...
    async ({ query, category, in_stock, limit, offset }) => {
      let rows = await searchProducts(env.DB, { query, category, inStock: in_stock, limit, offset });
      let queryMatched = true;

      if (query && rows.length === 0) {
        rows = await searchProducts(env.DB, { category, inStock: in_stock, limit, offset });
        queryMatched = false;
      }
